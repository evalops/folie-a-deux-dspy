...
class ExperimentResults:
    ...
    def add_round(self, round_num: int, acc_a: float, acc_b: float, 
                  agree_dev: float, agree_train: float) -> None:
...
def folie_a_deux(
    ...
    logger.info(f"Loaded {len(dev_labeled)} labeled examples, {len(train_unlabeled)} unlabeled")
    ...
    try:
        ...
        for round_num in range(1, config.rounds + 1):
            ...
            agree_train = agreement_rate(verifier_a, verifier_b, train_unlabeled[:train_sample_size])
            
            # Store results
            results.add_round(round_num, acc_a, acc_b, agree_dev, agree_train)
            
            # Log progress
            logger.info(f"[round {round_num}] accA={acc_a:.3f} accB={acc_b:.3f} "
                       f"agree_dev={agree_dev:.3f} agree_train={agree_train:.3f}")
            
            print(f"[round {round_num}] accA={acc_a:.3f} accB={acc_b:.3f} "
                  f"agree_dev={agree_dev:.3f} agree_train={agree_train:.3f}")
...
def run_ablation_study(base_config: ExperimentConfig = None) -> Dict[str, ExperimentResults]:
    """
