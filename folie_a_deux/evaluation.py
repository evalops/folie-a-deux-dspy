...
def evaluate(program, devset: List[dspy.Example], metric=None, display_progress: bool = True) -> float:
    """
...
def detailed_evaluation(program, devset: List[dspy.Example]) -> Dict[str, Any]:
    ...
    logger.info(f"Detailed evaluation: {results['correct']}/{results['total_examples']} correct, "
                f"{results['errors']} errors, accuracy={results['accuracy']:.3f}")
    
    return results


def compare_programs(program_a, program_b, devset: List[dspy.Example]) -> Dict[str, Any]:
    """
    ...
    for i, example in enumerate(devset):
        ...
        try:
            ...
            if "verdict" in example:
                ...
                elif not example_result["a_correct"] and not example_result["b_correct"]:
    ...
    logger.info(f"Program comparison: {results['agreement']}/{results['total_examples']} agree, "
